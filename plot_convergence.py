# import matplotlib.pyplot as plt
# import pandas as pd
#
#
# def run(results_directory, optimizer, objectivefunc, Iterations):
#     plt.ioff()
#     fileResultsData = pd.read_csv(results_directory + "/experiment.csv")
#
#     for j in range(0, len(objectivefunc)):
#         objective_name = objectivefunc[j]
#
#         startIteration = 0
#         if "SSA" in optimizer:
#             startIteration = 1
#         allGenerations = [x + 1 for x in range(startIteration, Iterations)]
#         for i in range(len(optimizer)):
#             optimizer_name = optimizer[i]
#
#             row = fileResultsData[
#                 (fileResultsData["Optimizer"] == optimizer_name)
#                 & (fileResultsData["objfname"] == objective_name)
#             ]
#             row = row.iloc[:, 3 + startIteration :]
#             plt.plot(allGenerations, row.values.tolist()[0], label=optimizer_name)
#         plt.xlabel("Iterations")
#         plt.ylabel("Fitness")
#         plt.legend(loc="upper right", bbox_to_anchor=(1.2, 1.02))
#         plt.grid()
#         fig_name = results_directory + "/convergence-" + objective_name + ".png"
#         plt.savefig(fig_name, bbox_inches="tight")
#         plt.clf()
#         # plt.show()

import matplotlib.pyplot as plt
import pandas as pd


# def run(results_directory, optimizer, objectivefunc, Iterations):
#     plt.ioff()
#     fileResultsData = pd.read_csv(results_directory + "/experiment.csv")
#
#     for j in range(0, len(objectivefunc)):
#         objective_name = objectivefunc[j]
#
#         startIteration = 0
#         if "SSA" in optimizer:
#             startIteration = 1
#         allGenerations = [x + 1 for x in range(startIteration, Iterations)]
#         plt.figure(figsize=(12, 8))  # Increase figure size for better visibility
#
#         for i in range(len(optimizer)):
#             optimizer_name = optimizer[i]
#
#             row = fileResultsData[
#                 (fileResultsData["Optimizer"] == optimizer_name)
#                 & (fileResultsData["objfname"] == objective_name)
#             ]
#             row = row.iloc[:, 3 + startIteration :]
#
#             plt.plot(
#                 allGenerations,
#                 row.values.tolist()[0],
#                 label=optimizer_name,
#                 linewidth=2.5,  # Thicker lines
#                 alpha=0.8,  # Slight transparency to distinguish overlapping lines
#                 # marker='o',  # Add markers for more detail
#                 markersize=6,  # Control marker size
#             )
#
#         plt.xlabel("Iterations", fontsize=14)
#         plt.ylabel("Fitness", fontsize=14)
#         plt.legend(loc="upper right", bbox_to_anchor=(1.0, 1.02), fontsize=12)
#         plt.grid(True)
#         plt.title(f"Convergence Plot: {objective_name}", fontsize=16)  # Add a plot title
#         fig_name = results_directory + "/convergence-" + objective_name + ".png"
#         plt.savefig(fig_name, bbox_inches="tight", dpi=300)  # Save with high resolution for better quality
#         plt.clf()

import pandas as pd
import matplotlib.pyplot as plt

import pandas as pd
import matplotlib.pyplot as plt

def run(results_directory, optimizer, objectivefunc, Iterations):
    plt.ioff()
    fileResultsData = pd.read_csv(results_directory + "/experiment.csv")

    # Define a list of colors (extend this list if you have more optimizers)
    colors = ['green', 'blue', 'red', 'purple', 'orange', 'cyan', 'magenta', '#DE92A8', '#a64d79']

    for j in range(0, len(objectivefunc)):
        objective_name = objectivefunc[j]

        startIteration = 0
        if "SSA" in optimizer:
            startIteration = 1
        allGenerations = [x + 1 for x in range(startIteration, Iterations)]
        plt.figure(figsize=(12, 8))  # Increase figure size for better visibility

        for i in range(len(optimizer)):
            optimizer_name = optimizer[i]

            row = fileResultsData[
                (fileResultsData["Optimizer"] == optimizer_name)
                & (fileResultsData["objfname"] == objective_name)
            ]
            row = row.iloc[:, 3 + startIteration :]

            plt.plot(
                allGenerations,
                row.values.tolist()[0],
                label=optimizer_name,
                linewidth=1.5,  # Thicker lines
                alpha=0.8,  # Slight transparency
                color=colors[i % len(colors)],  # Assign color to each optimizer
                markersize=6,  # Control marker size
            )

        # Set y-axis to logarithmic scale
        plt.yscale('log')

        plt.xlabel("Iterations", fontsize=14)
        plt.ylabel("Fitness", fontsize=14)
        plt.legend(loc="upper right", bbox_to_anchor=(1.0, 1.02), fontsize=12)
        plt.grid(True)
        plt.title(f"Convergence Plot: {objective_name}", fontsize=16)  # Add a plot title
        fig_name = results_directory + "/convergence-" + objective_name + ".png"
        plt.savefig(fig_name, bbox_inches="tight", dpi=300)  # Save with high resolution for better quality
        plt.clf()
