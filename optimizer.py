...
import Optimizers.ProposedMethod as pm
...
def selector(algo, func_details, popSize, Iter):
    ...
    if algo == "ProposedMethod":
        x = pm.ProposedMethod(getattr(benchmarks, function_name), lb, ub, dim, popSize, Iter)
    else:
        return null
    return x
...
def run(optimizer, objectivefunc, NumOfRuns, params, export_flags):
    ...
    results_directory = "Results/ArticleBoxPlot/" + time.strftime("%Y-%m-%d-%H-%M-%S") + "/"
                    ...
                    with open(ExportToFile, "a", newline="\n") as out:
                        ...
                        if (
                            ...
                            header = np.concatenate(
                                [["Optimizer", "objfname","Execution time", "Convergence Iter", "Value at Conv", "STOP Iteration"], CnvgHeader]
                            )
                            writer.writerow(header)
                            # print(convergence)
                            Flag_details = True  # at least one experiment
                        a = np.concatenate(
                            [[x.optimizer, x.objfname, x.executionTime, x.conviter, x.valueConv, x.stopiter], x.convergence]
                        )
                        writer.writerow(a)
                    out.close()
